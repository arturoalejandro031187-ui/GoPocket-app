    ...
    const load = async () => {
      try {
        ...
        let res: any = await supabase
          .from('listings')
          .select('id,title,description,price,images,public_id,is_featured,view_count,share_count')
          .eq('status', 'active')
          .eq('is_featured', true)
        ...
        if (res?.error) {
          ...
          if (code === '42703' || msg.toLowerCase().includes('does not exist')) {
            ...
            res = await supabase
              .from('listings')
              .select('id,title,description,price,images,public_id,is_featured')
              .eq('status', 'active')
              .eq('is_featured', true)
                ...
                return (
                  <Link key={p.id} href={`/listings/${p.id}`} className="group overflow-hidden rounded-3xl bg-white shadow-sm ring-1 ring-black/5 hover:shadow-md transition-shadow">
                    <div className="relative aspect-[4/5] bg-gray-100">
                      ...
                      <div className="absolute left-3 top-3 rounded-full bg-brand-pink px-3 py-1 text-xs font-extrabold text-white shadow">
                        Destacado
                      </div>
                    </div>
                    <div className="p-4">
