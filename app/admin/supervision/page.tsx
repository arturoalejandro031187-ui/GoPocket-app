...
function SupervisionContent() {
  ...
  const atencion = useMemo(() => {
    const s = summary;
    const items: { count: number; msg: string; href: string }[] = [];
    ...
    <div className="rounded-3xl bg-white/80 p-6 shadow-sm ring-1 ring-black/5 sm:p-8">
      ...
      <div className="mt-6 rounded-2xl border-2 border-amber-300 bg-amber-50 p-4 ring-2 ring-amber-200/60">
        <h2 className="flex items-center gap-2 text-base font-bold text-amber-900">
          ...
          {atencion.length > 0 ? 'Atención requerida — Qué debes revisar o resolver' : 'Estado de la plataforma'}
        </h2>
        {atencion.length > 0 ? (
          <ul className="mt-3 space-y-2">
            {atencion.map((a, i) => (
              <li key={i}>
                <Link
                  href={a.href}
                  className="flex flex-wrap items-center gap-2 rounded-xl border border-amber-200 bg-white px-4 py-3 text-sm font-medium text-amber-900 shadow-sm transition hover:border-amber-400 hover:bg-amber-100/80"
                >
