...
export async function GET(req: NextRequest) {
  try {
    ...
    if (!adminOk && order.buyerId !== guard.userId && order.sellerId !== guard.userId) {
...
export async function POST(req: NextRequest) {
  try {
    ...
    const senderRole =
      adminOk ? 'admin' : guard.userId === order.buyerId ? 'buyer' : guard.userId === order.sellerId ? 'seller' : 'user';
    ...
    let ins: any = await admin
      .from('order_messages')
      .insert([{ order_id: orderId, sender_id: guard.userId, sender_role: senderRole, body: message || '', attachments: atts }])
      .select('id,order_id,sender_id,sender_role,body,attachments,created_at')
      .single();
    ...
    if (ins.error) {
      ...
      if (code === '42703' || msg.includes('column')) {
        ...
        ins = await admin
          .from('order_messages')
          .insert([{ order_id: orderId, sender_id: guard.userId, body: message || '' }])
          .select('id,order_id,sender_id,body,created_at')
          .single();
      }
    }
    ...
    const recipients = [order.buyerId, order.sellerId].filter((id) => id && id !== guard.userId);
