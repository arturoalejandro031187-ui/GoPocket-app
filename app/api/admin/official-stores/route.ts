  ...
  try {
    ...
    try {
      const res = await admin
        .from('profiles')
        .select('id, full_name, email, is_official_store, official_store_name, official_store_brand_color, official_store_banner_url')
      ...
      const res = await admin
        .from('profiles')
        .select('id, full_name, is_official_store, official_store_name, official_store_brand_color, official_store_banner_url')
        .eq('is_official_store', true)
        .order('official_store_name', { ascending: true });
      data = res.data;
      error = res.error;
