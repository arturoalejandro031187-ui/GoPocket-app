  ...
  try {
    ...
    if (orderIds.length > 0 && (action === 'mark_paid' || action === 'sync_orders')) {
      ...
      if (oUpd.error) {
        ...
        // Enviar notificaciones por email a comprador y vendedores
        if (updatedOrders > 0) {
          try {
            console.log('[admin/offline-update] Enviando notificaciones de email...');
            // Notificar a comprador
            if (sessionRow?.buyer_id) {
              await notifyPaymentApprovedBuyer({
            ...
            await notifyPaymentApprovedSellers({
              admin: admin,
              orderIds: orderIds
            });
            console.log('[admin/offline-update] Notificaciones enviadas.');
          } catch (notifyErr) {
            console.error('[admin/offline-update] Error enviando notificaciones:', notifyErr);
