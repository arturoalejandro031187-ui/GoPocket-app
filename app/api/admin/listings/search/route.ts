...
export async function GET(req: NextRequest) {
  try {
    ...
    let query = admin
      .from('listings')
      .select(
        'id,public_id,title,price,currency,status,sale_type,seller_id,created_at,expires_at,view_count,auction_end_at,auction_highest_bid,auction_highest_bidder_id,is_deleted,deleted_at',
      )
    ...
    if (error) {
      ...
      if (code === '42703' || msg.toLowerCase().includes('does not exist')) {
        let q2 = admin
          .from('listings')
          .select('id,title,price,currency,status,sale_type,seller_id,created_at')
          .order('created_at', { ascending: false })
          .limit(limit);
        if (q) {
