...
export async function GET(req: NextRequest) {
  try {
    ...
    const mappedUsers = data
      .filter(user => {
        // Normalizar plan_type (si es null, string vacío o mixed case)
        const planType = (user.plan_type || '').toLowerCase();
        const isPlanPro = planType.includes('pro'); // Allow 'pro', 'super-pro', etc.
        
        // Verificar historial de suscripción (active or expired)
        // We want to return anyone who IS pro or WAS pro (has a start date)
        // so the frontend can show expired users too.
      ...
      .map(user => ({
        ...
        email: user.email,
        is_pro: (user.plan_type || '').toLowerCase().includes('pro') || (user.pro_subscription_end && new Date(user.pro_subscription_end) > now),
