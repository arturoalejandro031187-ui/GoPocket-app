...
export async function POST(req: NextRequest) {
  try {
    ...
    for (const order of orders) {
      ...
      // a) Actualizar método de pago si es diferente
      ...
      if (updateErr) {
        console.error('[PAY-ORDER] Error updating order status after deduction:', updateErr);
        // Continuamos con las siguientes, aunque esto es peligroso si falla. 
        // Idealmente usaríamos una transacción de base de datos, pero Supabase JS client no soporta transacciones complejas fácilmente.
      }
    }
