...
export async function POST(req: NextRequest) {
  try {
    ...
    const { data: order, error: oErr } = await admin.from('orders').select('id,buyer_id,seller_id,status,shipping_label_url').eq('id', orderId).maybeSingle();
    ...
    if (sellerId !== userData.user.id) return NextResponse.json({ error: 'No autorizado.' }, { status: 403 });
    
    // Permitir calificar cuando hay guía subida O cuando está completado/entregado
