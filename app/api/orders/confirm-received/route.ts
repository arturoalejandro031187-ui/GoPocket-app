...
export async function POST(req: NextRequest) {
  try {
    ...
    const { data: order, error: oErr } = await admin.from('orders').select('id,buyer_id,seller_id,status,total').eq('id', orderId).maybeSingle();
    ...
    if (buyerId !== userData.user.id) return NextResponse.json({ error: 'No autorizado.' }, { status: 403 });
    ...
    // 1) Si shipped → delivered; liberar dinero (paid_to_seller_at) solo si aún no está liberado (estable, no sobrescribir).
    ...
    const patch: any = status === 'shipped' ? { status: 'delivered', paid_to_seller_at: now, paid_to_seller_by: buyerId } : {};
