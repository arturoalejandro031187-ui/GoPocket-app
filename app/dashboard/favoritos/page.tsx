  ...
  useEffect(() => {
    ...
    const boot = async () => {
      try {
        ...
        if (ids.length > 0) {
          const listRes: any = await supabase.from('listings').select('id,title,price,images,public_id').in('id', ids);
          ...
          {isBooting ? (
            ...
            <div className="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
              {favorites.map((f) => {
                ...
                return (
                  <Link key={f.id} href={`/listings/${f.listing_id}`} className="group overflow-hidden rounded-3xl bg-white shadow-sm ring-1 ring-black/5 hover:shadow-md transition-shadow">
                    ...
                    <div className="p-4">
                      <div className="line-clamp-1 text-sm font-semibold text-gray-900">{String(l?.title || 'Publicación')}</div>
                      {l?.public_id ? <div className="mt-1 text-xs text-gray-500">ID: {String(l.public_id)}</div> : null}
                    </div>
                  </Link>
                );
              })}
