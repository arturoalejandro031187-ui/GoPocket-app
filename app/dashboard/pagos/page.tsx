...
export default function DashboardPagosPage() {
      ...
      try {
        ...
        try {
          const disputesRes = await supabase
            ...
            .eq('seller_id', user.id)
            .eq('status', 'open');
          const raw = Array.isArray(disputesRes?.data) ? disputesRes.data : [];
          disputedIds = raw.map((d: any) => String(d?.order_id || '').trim()).filter(Boolean);
        } catch {
          disputedIds = [];
        }
        if (!cancelled) setDisputedOrderIds(disputedIds);
  ...
  const summary = useMemo(() => {
    ...
    const disputedOrders = active.filter((o) => disputedSet.has(String(o?.id || '').trim()));
    const amountRetenidoPorDisputas = sum(disputedOrders);

    // Calcular montos
    const amountReleased = sum(released);
    const amountToReleaseRaw = sum(toRelease);
  ...
  }, [orders, disputedOrderIds, guideDeductionTotal]);
  ...
  const rows = useMemo(() => {
    ...
    if (tab === 'por_liberar') return list.filter((o) => isPaidStatus(normStatus(o?.status)) && !isReleasedStatus(normStatus(o?.status)));
    if (tab === 'todo') return list;
    return list;
  }, [orders, itemsByOrder, q, tab]);
          ...
          <div className="mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
            ...
            {summary.guideDeductionTotal > 0 ? (
              <div className="rounded-2xl border border-amber-200 bg-amber-50 px-4 py-3 ring-1 ring-amber-200">
                ...
                <div className="mt-1 text-xs text-amber-900/80">
                  Se descontará de tu saldo por guías de devolución asignadas con cargo al vendedor.
                </div>
              </div>
            ) : null}
          </div>
