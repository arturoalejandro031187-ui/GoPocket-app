...
export default function DashboardSoporteChatPage() {
  ...
  const load = async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/support/messages?conversationId=${encodeURIComponent(conversationId)}&limit=300&t=${Date.now()}`, {
            ...
            {isBooting ? (
              ...
              <div className="space-y-2">
                {messages.map((m) => {
                  ...
                  const tone = mine ? 'mine' : isAdmin ? 'admin' : 'other';
                  const styles =
                    tone === 'mine'
