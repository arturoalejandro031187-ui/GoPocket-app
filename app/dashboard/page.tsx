  ...
  useEffect(() => {
    ...
    const boot = async () => {
      try {
        ...
        setDisplayName(metaFullName || metaUsername || emailName || 'Usuario');
        ...
        // Traer perfil (si existe) para nombre, documentos (INE) y estado del tour
        const { data: profile, error: profileError } = await supabase
          .from('profiles')
          .select('full_name, ine_front_url, ine_back_url, has_seen_onboarding_tour')
          .eq('id', user.id)
          .maybeSingle();

        if (!cancelled && profileError) {
          const anyErr = profileError as any;
          const code = String(anyErr?.code || '');
          const msg = String(anyErr?.message || '');
          if (code === '42703' && msg.includes('ine_front_url')) {
            setPageError(
              "Tu tabla `profiles` no tiene `ine_front_url`/`ine_back_url`. Ejecuta `supabase_profiles_ine_migration.sql` en Supabase.",
            );
          }
        }
        ...
        if (!cancelled && !profileError && profile) {
          ...
          const docs = [front, back].filter((x) => x && x.length > 0).length;
          setDocumentsUploaded(docs);
          // Cargar estado del tour (si es null o false, el tour está activo)
          setHasSeenTour((profile as any).has_seen_onboarding_tour === true);
        } else if (!cancelled && !profileError) {
          // Si no hay perfil, el tour está activo por defecto (null = activo)
          setHasSeenTour(false);
        }

        // Importante: no bloquear el "boot" del dashboard por notificaciones/preguntas
        // (si Supabase está lento, se quedaba en skeleton).
        await loadOperations(user.id);
        void loadAlerts(user.id);
        void loadUnansweredQuestionsCount(user.id);
        void loadSalesNotifCount(user.id);
