...
export default function CheckoutPage() {
  ...
  const shippingModeSummary = useMemo(() => {
    ...
    // Detectar presencia de modos por vendedor vs GoPocket
    let anySellerManaged = false;
    let anyGoPocket = false;
    for (const ci of cartItems) {
      const l = listingsById[ci.listing_id];
      if (!l) continue;
      if (l.shipping_by_seller) anySellerManaged = true;
      else anyGoPocket = true;
    }
    if (anySellerManaged && anyGoPocket) {
      return { label: 'Mixto: GoPocket + Vendedor', tone: 'mixed' as const };
    }
    if (anySellerManaged) {
