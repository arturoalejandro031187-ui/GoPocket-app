...
export function scanListingContentPolicy(input: {
  ...
  const addressKeyword =
    /\b(calle|av\.?|avenida|colonia|fracc\.?|fraccionamiento|cp|c\.p\.|c[oó]digo postal|postal|domicilio|direcci[oó]n|n[uú]mero|no\.|num\.?|interior|ext\.?|exterior|entre|esquina)\b/i;
  if (addressKeyword.test(text) && /\d/.test(text)) {
