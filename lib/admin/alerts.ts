...
function createOrderAlert(order: Order): AdminAlert | null {
  ...
  return {
    ...
    category: 'logistics',
    title: isUrgent
      ? `🚨 Orden GoPocket sin guía > 24h: ${order.id.slice(0, 8)}`
      : `⚠️ Orden GoPocket sin guía: ${order.id.slice(0, 8)}`,
    description: `Orden GoPocket pagada hace ${Math.round(hoursSincePaid)}h sin guía de envío`,
